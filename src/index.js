...
results = Vue.component('items-results', {
    ...
    <div>
        ...
        <p v-else-if="items[0].value=='Empty'">-</p>
        ...
        <div v-else>
                <ul>
                    <li v-for="item in items">
                        <a href="javascript:void(0)" v-on:click="gotoArticle( item.value.value )">{{item.valueLabel.value}}</a>
                    </li>
                </ul>
        </div>
    </div>
...
viewallitems = Vue.component('view-all-items', {
    ...
    <div>
        <div class="header">
            <p style="margin-bottom:0px"><b><i class="fas fa-search"></i></b> {{ classLabel }} </p>
            <a :href="window.location.href+'&view=subclass'" onclick="return false;" class="classOptions" @click="changeView('superclass')"><i title="more general class" class="fas fa-arrow-up"></i></a>
            <b>&nbsp;&nbsp;</b>
            <a :href="window.location.href+'&view=superclass'" onclick="return false;" class="classOptions" @click="changeView('subclass')"><i title="more specific class" class="fas fa-arrow-down"></i></a>
        ...
        <div class="content" id="viewallitems">
                ...
                <!--p v-else-if="filtersCount==0">-</p-->
                <div class="filter-box" v-else-if="filtersCount<40">
                    <b><i title="filters" class="fas fa-filter"></i></b> <span v-for="filter in filters"><a :href="window.location.href+'&cf='+filter.value.value.split('/').slice(-1)[0]" onclick="return false;" @click="showFilter(filter)">{{filter.valueLabel.value}}</a> <b v-if="filters[filtersCount-1].valueLabel.value != filter.valueLabel.value">&middot; </b> </span>
                </div>
                <p v-else><a class="classOptions" @click="changeView('filters-view')">Add a filter</a></p>
            <p><img v-if="totalValues==''" src='loading.gif'></p>
            <p v-if="totalValues>0"><b>{{ totalValues<1000000?numberWithCommas(totalValues):"1 million +" }}</b> items</p>
...
filtersview = Vue.component('filters-view', {
    ...
    <div>
        <div class="header">
            <p><b><i class="fas fa-search"></i></b> {{ classLabel }} </p>
            <p><b><i title="filters" class="fas fa-filter"></i></b>:</p>
            <!--p v-if="!appliedFilters.length && !appliedRanges.length && !appliedQuantities">-</p-->
            <ul>
        ...
        <div class="content">
            ...
            <!--p v-else-if="filters[0].value=='Empty'">-</p-->
            <div v-else>
                <p><b>{{ totalValues<1000000?numberWithCommas(totalValues):"1 million +" }}</b> items</p>
...
filtervalues = Vue.component('filter-values', {
    ...
    <div>
        <div class="header">
            <p><b><i class="fas fa-search"></i></b> {{ classLabel }} </p>
            <p><b><i title="filters" class="fas fa-filter"></i></b>:</p>
            <!--p v-if="!appliedFilters.length && !appliedRanges.length && !appliedQuantities.length">-</p-->
            <p v-for="filter in appliedFilters"><b>{{filter.filterValueLabel}}</b>: <span v-if="filter.value == 'novalue'" :style="{ fontStyle: 'italic' }">{{ filter.valueLabel }}</span><span v-else>{{ filter.valueLabel }}</span> (<a @click="removeFilter(filter)">X</a>)</p>
            <p v-for="range in appliedRanges"><b>{{range.filterValueLabel}}</b>: <span v-if="range.valueLL == 'novalue'" :style="{ fontStyle: 'italic' }">{{ range.valueLabel }}</span><span v-else>{{ range.valueLabel }}</span> (<a @click="removeRange(range)">X</a>)</p>
            <p v-for="quantity in appliedQuantities"><b>{{quantity.filterValueLabel}}</b>: <span v-if="quantity.valueLL == 'novalue'" :style="{ fontStyle: 'italic' }">{{ quantity.valueLabel }}</span><span v-else>{{ quantity.valueLabel }}</span> {{quantity.unit}}(<a @click="removeQuantity(quantity)">X</a>)</p>
        </div>
        <div class="content">
            <div v-if="itemsType==''"><p><i class="fas fa-hourglass-half"></i> <b>{{currentFilter.valueLabel}}</b> ...</p><img src='loading.gif'></div>
            ...
            <div v-else-if="itemsType=='Item'">
                <p v-if="totalValues!=''"><b>{{ totalValues<1000000?numberWithCommas(totalValues):"1 million +" }}</b> items</p>
                <p> <i class="far fa-list-alt"></i> {{ appliedFilters.findIndex(filter => filter.filterValue == currentFilter.value) !=-1?" + ":""}} <b>{{currentFilter.valueLabel}}</b>: </p>
            ...
            <div v-else-if="itemsType=='ItemFail'">
                ...
                <p> <i class="far fa-list-alt"></i> <b>{{currentFilter.valueLabel}}</b>:</p>
                <ul>
            ...
            <div v-else-if="itemsType=='Quantity'">
                <p v-if="displayCount == 1 && totalValues!=''"><b>{{ totalValues<1000000?numberWithCommas(totalValues):"1 million +" }}</b> items</p>
...
subclass = Vue.component('subclass-view', {
    ...
    <div>
        ...
        <p><b>"{{ classLabel }}" <i class="fas fa-arrow-down"></i></b><p>
        <div class="content">
            <img v-if="!items.length" src='loading.gif'>
            <p v-else-if="items[0].value=='Empty'">-</p>
            <div v-else>
